...
function getCity(coordinates) {
  ...
  xhr.open(
    'GET',
    'https://us1.locationiq.com/v1/reverse.php?key=pk.362593489846339f3055b241a316287a&lat=' +
      lat +
      '&lon=' +
      lng +
      '&format=json',
...
document.getElementById('myInput').addEventListener(
  ...
  debounce(() => {
    ...
    if (search === '') {
      ...
      fetch(
        `https://api.locationiq.com/v1/autocomplete?key=pk.362593489846339f3055b241a316287a&q=${search}&limit=10&dedupe=1&tag=place:city`
      )
        .then((response) => {
      ...
      function convert(obj) {
        if (obj !== undefined) {
          let array = [];
          for (let j = 0; j < obj.length; j++) array[j] = obj[j].display_place;
...
function autocomplete(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (
      arr[i]
        .substr(0, document.getElementById('myInput').value.length)
        .toUpperCase() == document.getElementById('myInput').value.toUpperCase()
    ) {
      ...
      b.innerHTML =
        '<strong>' +
        arr[i].substr(0, document.getElementById('myInput').value.length) +
        '</strong>';
      ...
      b.addEventListener('click', (eve) => {
        document.getElementById('myInput').value =
          eve.target.lastElementChild.value;
